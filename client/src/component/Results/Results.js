...
function displayWinner(candidates) {
    ...
    if (winner.length === 1) {
      ...
      return (
        <div className="container-winner">
          <div className="winner-info">
            <p className="winner-tag">Winner!</p>
            <h2> {winner[0].header}</h2>
            <p className="winner-slogan">{winner[0].slogan}</p>
          </div>
          <div className="winner-votes">
            <div className="votes-tag">Total Votes: </div>
            <div className="vote-count">{winner[0].voteCount}</div>
          </div>
        </div>
      );
    } else {
      ...
      return (
        <div className="container-winner">
          <div className="winner-info">
            <p className="winner-tag">It is a Tie! between:</p>
            {/* <p className="tie-message">It's a tie between:</p> */}
            {winner.map((candidate, index) => (
              <div key={index}>
                <h2>{candidate.header}</h2>
                {/* <p className="winner-slogan">{candidate.slogan}</p> */}
              </div>
            ))}
          </div>
          <div className="winner-votes">
            <div className="votes-tag">Total Votes:</div>
            {winner.map((candidate, index) => (
              <div key={index} className="vote-count">
                {candidate.voteCount}
              </div>
            ))}
          </div>
  ...
  const winners = getWinners(candidates);
  return <>{renderWinner(winners)}</>;
}
